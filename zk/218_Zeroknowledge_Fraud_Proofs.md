...
在设计 Rollup（聚合链）时，一个关键的设计考虑因素是如何在提高底层 Layer 1 的可扩展性的同时确保安全性和可信。对于乐观 Rollup，安全性是通过欺诈证明来确保的：这是一种证明聚合链级别执行出现错误并且状态必须回滚的证据。

与现有的乐观 Rollup不同，Layer N 不依赖于在链上重放交易进行欺诈证明。相反，Layer N 采用一种新方法，利用零知识证明和 RISC Zero 的零知识虚拟机进行欺诈证明。

## A primer on replay proofs重放证明简介

乐观 Rollup将状态更新和将先前状态迁移到更新状态的相应交易一起发布到底层的 L1。假设我们作为 Rollup的验证者，声称我们观察到发布到以太坊的最终状态是无效的（换句话说，更新的状态与Rollup发布到DA的交易不相符）。在这种情况下，我们会提交一份欺诈证明，如果被接受，将会获得可观的经济奖励。

对于欺诈证明来说，最简单的方法是让一个智能合约在以太坊（L1）上重新执行交易，并检查生成的状态是否准确，我们将其称为“简单重放证明”。

如果区块很大，这种方法将变得非常昂贵。然而，我们可以做出一个很好的观察：如果交易不能生成预期的状态，那么在某个点上执行了错误的指令。“交互式欺诈证明”就是找到这个指令。为了构建交互式欺诈证明，验证者通过用户和操作者之间的一系列挑战进行二分搜索，在每一步都将搜索空间一分为二。一旦验证者找出了第一个执行错误的指令，智能合约就会重新执行它，并检查是否正确执行。这种巧妙的技术就是Arbitrum所说的解剖(dissection)，它本质上是对我们介绍的重放证明的扩展。

然而，这引发了一个重要的问题：我们如何确保链上执行和链下执行的行为完全相同？

## Difficulties with Replay Proofs重放证明的难点

简单重放证明和交互证明都面临一个关键约束，即指令必须能够在基础层和Rollup上以相同的方式执行。换句话说，两种实现都需要使用相同的虚拟机（VM），并确保行为相匹配。

就Optimism而言，他们之前的实现是以Geth为基础，对以太坊虚拟机进行了微调，称之为Optimism虚拟机（OVM）。最近，他们还开发了一个基于Solidity的链上MIPS指令模拟器，用于运行Minigeth解释器，从而可以模拟和验证EVM状态转换。而Arbitrum则使用了修改版的WASM，称之为WAVM[1](https://www.layern.com/blog/zkfp#user-content-fn-1). 这种设计意味着Optimism和Arbitrum可以分别支持任何针对MIPS和WASM的语言。

然而，对于Optimism和Arbitrum而言，这意味着它们各自的虚拟机需要在Solidity中实现，以便以太坊能够进行模拟。不仅如此，每个实现还需要具有完全相同的行为。对于像Optimism这样的非交互式证明，Gas成本也显著较高，因为我们需要重放区块中的每个交易。

## Enter RISC Zero  引入RISC Zero  RISC Zero登场
...
## Beyond optimistic rollups 超越乐观 Rollup
